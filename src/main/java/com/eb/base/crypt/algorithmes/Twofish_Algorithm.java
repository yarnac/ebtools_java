...
import com.eb.base.crypt.ICrypter;
 ...
 * Reference:
 * <ol>
 * <li>TWOFISH2.C -- Optimized C API calls for TWOFISH AES submission, Version 1.00, April 1998, by
 * Doug Whiting.
 * </ol>
 * <p>
 * 
 * <b>Copyright</b> &copy; 1998 <a href="http://www.systemics.com/">Systemics Ltd</a> on behalf of
 * the <a href="http://www.systemics.com/docs/cryptix/">Cryptix Development Team</a>. <br>
 * All rights reserved.
 * <p>
 * 
 * <b>$Revision: $</b>
 ...
 implements ICrypter
{
	...
	static {
		...
		for (i = 0; i < 256; i++) {
			...
			MDS[0][i] = m1[P_00] | // fill matrix w/ above elements
					mX[P_00] << 8 |
					mY[P_00] << 16 |
					mY[P_00] << 24;
			MDS[1][i] = mY[P_10] |
	...
	public static synchronized Object makeKey(byte[] k)
		...
		if (DEBUG)
			trace(IN, "makeKey(" + Arrays.toString(k).toString() + ")");
		if (k == null)
		...
		if (DEBUG && debuglevel > 7) {
			...
			for (i = 0; i < k64Cnt; i++) {
				System.out.println("0x" + intToString(k32o[i]) + "  0x" + intToString(k32e[i])
						+ " --> 0x" + intToString(sBoxKey[k64Cnt - 1 - i]));
			}
			System.out.println();
			System.out.println("Round keys:");
			for (i = 0; i < ROUND_SUBKEYS + 2 * ROUNDS; i += 2) {
				System.out.println("0x" + intToString(subKeys[i]) + "  0x"
						+ intToString(subKeys[i + 1]));
			}
			System.out.println();
	...
	public static byte[]
		...
		if (DEBUG && debuglevel > 6)
			System.out.println("PTw=" + intToString(x0) + intToString(x1) + intToString(x2)
					+ intToString(x3));
		...
		for (int R = 0; R < ROUNDS; R += 2) {
			...
			x3 ^= t0 + 2 * t1 + sKey[k++];
			if (DEBUG && debuglevel > 6)
				System.out.println("CT" + (R) + "=" + intToString(x0) + intToString(x1)
						+ intToString(x2) + intToString(x3));
			...
			x1 ^= t0 + 2 * t1 + sKey[k++];
			if (DEBUG && debuglevel > 6)
				System.out.println("CT" + (R + 1) + "=" + intToString(x0) + intToString(x1)
						+ intToString(x2) + intToString(x3));
		}
		...
		if (DEBUG && debuglevel > 6)
			System.out.println("CTw=" + intToString(x0) + intToString(x1) + intToString(x2)
					+ intToString(x3));
	...
	public static byte[]
		...
		if (DEBUG)
			trace(IN, "blockDecrypt(" + Arrays.toString(in) + ", " + inOffset + ", " + sessionKey + ")");
		Object[] sk = (Object[]) sessionKey; // extract S-box and session key
		int[] sBox = (int[]) sk[0];
		int[] sKey = (int[]) sk[1];
		...
		x1 ^= sKey[OUTPUT_WHITEN + 3];
		if (DEBUG && debuglevel > 6)
			System.out.println("CTw=" + intToString(x2) + intToString(x3) + intToString(x0)
					+ intToString(x1));
		...
		for (int R = 0; R < ROUNDS; R += 2) {
			...
			x0 ^= t0 + t1 + sKey[k--];
			if (DEBUG && debuglevel > 6)
				System.out.println("PT" + (ROUNDS - R) + "=" + intToString(x2) + intToString(x3)
			...
			x2 ^= t0 + t1 + sKey[k--];
			if (DEBUG && debuglevel > 6)
				System.out.println("PT" + (ROUNDS - R - 1) + "=" + intToString(x2)
						+ intToString(x3) + intToString(x0) + intToString(x1));
		}
		x0 ^= sKey[INPUT_WHITEN];
		x1 ^= sKey[INPUT_WHITEN + 1];
		x2 ^= sKey[INPUT_WHITEN + 2];
		x3 ^= sKey[INPUT_WHITEN + 3];
		if (DEBUG && debuglevel > 6)
			System.out.println("PTw=" + intToString(x2) + intToString(x3) + intToString(x0)
					+ intToString(x1));
